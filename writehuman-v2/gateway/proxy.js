...
// WriteHuman V2: the "backend" is IN-PROCESS (session manager), not a remote HTTP service.
// server.js injects it via setBackend(); backendPost above is left intact but unused. The
// injected backend returns the SAME { status, body } shape the HTTP backend used to, so the
// rest of this gateway is unchanged.
let __backend = null;
function setBackend(b) { __backend = b; }
function backendValidate(token) {
  return __backend ? __backend.validate(token) : Promise.resolve({ status: 0, body: {} });
}
function gatewayApiPost(subpath, token, jsonBody) {
  if (!GATEWAY_KEY) return Promise.resolve({ status: 0, body: {} });
  return __backend ? __backend.call(subpath, token, jsonBody) : Promise.resolve({ status: 0, body: {} });
}
...
// WriteHuman V2: overlay assets live in the V2 root /public (one level up from gateway/).
...
// ── Request handler ─────────────────────────────────────────────────────────────
// WriteHuman V2: exported as a handler (server.js owns the http.Server and routes /v2/*
// to the in-process backend before delegating everything else here).
...
module.exports = { handle, setBackend };
