...
import { Button, Card, Empty, ErrorState, Field, Loading, Modal, PageHeader, SearchBox, Select, Status } from '../components/ui';
...
export default function AccessPage(){
  ...
  const [q,setQ]=useState(''); const [saveError,setSaveError]=useState(''); const [editor,setEditor]=useState(null);
  const dq=useDebouncedValue(q);
  // Filtering happens on the server so this page behaves like every other CRM list, and so a large
  // team does not have to be shipped to the browser before it can be narrowed down.
  const resource=useResource(()=>`/admin/access?search=${encodeURIComponent(dq)}`,[dq]);
  const data=resource.data; const load=resource.reload; const error=saveError||resource.error;
  ...
  if(resource.initialLoading)return <Loading label="Loading team access…"/>;if(!data&&error)return <ErrorState message={error} onRetry={load}/>;
