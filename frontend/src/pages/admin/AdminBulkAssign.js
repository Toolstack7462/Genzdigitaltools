            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1.5 max-h-48 overflow-y-auto pr-1">
              {filteredTools.map(tool => {
                ...
                return (
                  ...
                  >
                    <Package size={14} className={`flex-shrink-0 ${isSelected ? 'text-genz-teal' : 'text-genz-muted'}`} />
                    <span className="text-[12.5px] font-medium text-genz-navy truncate flex-1">{tool.name}</span>
                    {isSelected && <CheckCircle2 size={13} className="text-genz-teal flex-shrink-0" />}
                  </button>
                );
              })}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 max-h-48 overflow-y-auto pr-1">
              {filteredClients.map(client => {
                ...
                return (
                  ...
                  >
                    <span className={`w-6 h-6 rounded-md flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0 ${isSelected ? '' : 'opacity-80'}`}
                          style={{ background: 'var(--gradient-cta)' }}>
                      {client.fullName?.charAt(0) || '?'}
                    </span>
                    <div className="flex-1 min-w-0">
                      <p className="text-[12.5px] font-medium text-genz-navy truncate leading-tight">{client.fullName}</p>
