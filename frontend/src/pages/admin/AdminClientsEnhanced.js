...
const AdminClientsEnhanced = () => {
  ...
  const ActionBtn = ({ onClick, title, tone, icon: Icon, label, testId }) => {
    ...
    return (
      <button
        ...
        aria-label={title}
        data-testid={testId}
        className={`inline-flex items-center justify-center gap-1.5 h-7 ${label ? 'px-2' : 'w-7'} rounded-md border border-genz-border bg-genz-bg ${tones[tone]} transition-colors text-[12px] font-medium`}
      >
        <Icon size={13} />
  ...
  const Avatar = ({ client, size = 'sm' }) => (
    <div
      className={`${size === 'sm' ? 'w-7 h-7 text-xs' : 'w-9 h-9 text-sm'} rounded-md flex items-center justify-center flex-shrink-0 text-white font-bold`}
          ...
          <>
            ...
            <div className={`${ADMIN_CARD_VARIANTS.default} rounded-2xl overflow-hidden hidden md:block`}>
              <div className="overflow-x-auto max-h-[calc(100vh-280px)]">
                <table className="ds-table ds-table-compact" data-testid="admin-clients-table">
                  <thead>
                    <tr>
                      <th>Client</th>
                      <th>Status</th>
                  ...
                  <tbody>
                    {clients.map(client => (
                      <tr key={client._id} data-testid={`client-card-${client._id}`}>
                        <td>
                          <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                              <p className="font-semibold text-genz-navy text-[13px] truncate leading-tight">{client.fullName}</p>
                              <p className="text-[11px] text-genz-muted truncate leading-tight">{client.email}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="whitespace-nowrap text-[11.5px] text-genz-muted">{formatDate(client.lastLoginAt)}</td>
