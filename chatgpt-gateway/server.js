...
// Decompress an upstream body for the branches that must read/rewrite it (HTML/JSON).
// Piped binary/asset bodies are NOT decompressed here — they pass through compressed
// (content-encoding preserved) so the browser inflates them, saving 3-4x on transfer.
function maybeDecompress(buf, encoding) {
  const enc = String(encoding || '').toLowerCase();
  try {
    if (enc.includes('br')) return zlib.brotliDecompressSync(buf);
    if (enc.includes('gzip')) return zlib.gunzipSync(buf);
    if (enc.includes('deflate')) { try { return zlib.inflateSync(buf); } catch (_) { return zlib.inflateRawSync(buf); } }
  } catch (_) { /* fall through: serve raw bytes rather than a broken decode */ }
  return buf;
}
...
const STRIP_REQ_HEADERS = [
  ...
  // Hop-by-hop request headers — let Node's agent manage the upstream connection.
  // (The WebSocket upgrade handler re-adds connection/upgrade/sec-websocket-* itself.)
...
const STRIP_RESP_HEADERS = new Set([
  ...
  // Hop-by-hop headers (RFC 7230 §6.1). These MUST NOT be relayed: when LiteSpeed →
  // Cloudflare re-serve the response over HTTP/2, RFC 7540 §8.1.2.2 forbids them and the
  // browser aborts the stream with ERR_HTTP2_PROTOCOL_ERROR / ERR_CONNECTION_CLOSED.
  // chatgpt.com's HTTP/1.1 upstream sends `Connection: keep-alive`, which is what broke
  // every /backend-api/* fetch even though the gateway itself received a clean 200.
...
// For CLIENT leases: do NOT relay the upstream's account Set-Cookie to the browser.
// The gateway is stateful — it injects the VAULT account cookies upstream on every
// request — so the browser never needs ChatGPT's cookies. Relaying them made the
// browser accumulate ~25-30 host cookies (incl. a large session JWT); it then sent
// them all back and the request header list blew past LiteSpeed/Cloudflare's HTTP/2
// MAX_HEADER_LIST_SIZE (~16KB) → the stream was reset BEFORE reaching Node
// (ERR_HTTP2_PROTOCOL_ERROR / ERR_CONNECTION_CLOSED). Keep ONLY the small Cloudflare
// cookies so cf_clearance/__cf_bm can round-trip for the challenge pass-through.
// (CF_COOKIE_RE is defined earlier; it matches Cloudflare cookie names at string start.)
function filterClientSetCookie(values) {
  return [].concat(values || [])
    .filter(v => CF_COOKIE_RE.test(String(v).trimStart()))
...
// Keep the browser's cookie jar TINY. ChatGPT re-inflates it via client-side
// document.cookie writes (Statsig / Datadog / oai telemetry). Those cookies are then
// sent back on every request and, together with the auth + sentinel headers, blow past
// Hostinger LiteSpeed's ~10KB HTTP/2 request-header cap → the large send requests get a
// 400 before they even reach the gateway. Auth is injected from the vault UPSTREAM, so
// the browser needs none of these cookies; keep only our lease cookie + Cloudflare's.
// The shim (a) drops non-essential document.cookie writes and (b) sweeps the jar right
// before each same-origin API fetch/XHR so nothing bloats the request headers.
function injectCookieMinimizer(html, ctx) {
  if (!ctx || ctx.capture || ctx.asset) return html; // never during capture / asset proxy
  const keepSrc = JSON.stringify(String(LEASE_COOKIE || 'pg_lease'));
  const script = '<script>(function(){try{'
    + 'var LEASE=' + keepSrc + ';'
    + 'var KEEP=new RegExp("^(?:"+LEASE.replace(/[.*+?^${}()|[\\]\\\\]/g,"\\\\$&")+"|cf_clearance|__cf_bm|__cf_chl|cf_chl|__cflb|cf_ob_info|cf_use_ob)$","i");'
    + 'function nm(s){var i=s.indexOf("=");return (i<0?s:s.slice(0,i)).trim();}'
    + 'function sweep(){try{var cs=document.cookie?document.cookie.split(";"):[];for(var i=0;i<cs.length;i++){var n=nm(cs[i]);if(n&&!KEEP.test(n)){document.cookie=n+"=; Path=/; Max-Age=0; SameSite=Lax";document.cookie=n+"=; Path=/; Max-Age=0";}}}catch(e){}}'
    + 'try{var proto=Document.prototype||HTMLDocument.prototype;var d=Object.getOwnPropertyDescriptor(proto,"cookie");'
    + 'if(d&&d.get&&d.set&&d.configurable){Object.defineProperty(document,"cookie",{configurable:true,enumerable:true,'
    + 'get:function(){return d.get.call(document);},'
    + 'set:function(v){try{var n=nm(String(v));if(n&&!KEEP.test(n))return true;}catch(e){}return d.set.call(document,String(v));}});}}catch(e){}'
    + 'sweep();'
    + 'var TEL=/^https:\\/\\/chatgpt\\.com\\/(?:ces|statsc)\\b/i;'
    + 'try{var _f=window.fetch;if(_f){window.fetch=function(a){try{var u=(typeof a==="string")?a:(a&&a.url)||"";'
    + 'if(TEL.test(u)){return Promise.resolve(new Response("{}",{status:200,headers:{"content-type":"application/json"}}));}'  // no-op non-essential cross-origin telemetry (kills CORS spam + retry overhead)
    + 'if(!/^https?:/i.test(u)||u.indexOf(location.origin)===0){sweep();}}catch(e){}return _f.apply(this,arguments);};}}catch(e){}'
    + 'try{var _o=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(m,u){try{if(typeof u==="string"&&(!/^https?:/i.test(u)||u.indexOf(location.origin)===0))sweep();}catch(e){}return _o.apply(this,arguments);};}catch(e){}'
...
function buildUpstreamHeaders(req, upURL, session, minimal) {
  ...
  // Ask upstream to COMPRESS. Piped assets pass the compressed bytes straight to the
  // browser (big speed win); HTML/JSON we rewrite are decompressed in-process first.
  // (Also a more realistic fingerprint than the old forced `identity`.)
    ...
    const upstream = upLib.request(`${upURL.origin}${effUpPath}`, { method: req.method, headers }, (uRes) => {
      ...
      for (const [k, v] of Object.entries(uRes.headers)) {
        ...
        if (k.toLowerCase() === 'set-cookie') {
          // Capture mode needs the cookies (to save the session); client leases must NOT
          // accumulate ANY upstream cookie — the browser then sends them all back and the
          // request header list exceeds LiteSpeed/Cloudflare's HTTP/2 limit (~8-16KB),
          // which killed the large send requests (sentinel proof tokens). The gateway
          // injects the vault cookies upstream itself, so the browser needs none of them.
          if (ctx.capture) outHeaders[k] = rewriteSetCookie(v);
          // else: drop entirely (client lease keeps only its own __Host lease cookie).
          continue;
        }
        if (k.toLowerCase() === 'location' && typeof v === 'string') { outHeaders[k] = rewriteUpstreamUrls(v).text; continue; }
        outHeaders[k] = v;
      }
      // TEMP DIAGNOSTIC (sanitized — header NAMES only, never values/cookies/tokens):
      // confirms no hop-by-hop header (connection/keep-alive/transfer-encoding/upgrade)
      // is being relayed into the HTTP/2 response, and flags any stream abort.
      if (process.env.PROXY_LOG_ALL === '1' && /\/backend-api\//.test(reqPathOnly)) {
        let reqHdrBytes = 0; try { for (const [hk, hv] of Object.entries(req.headers)) reqHdrBytes += hk.length + String(hv).length + 4; } catch (_) {}
        const cookieRaw = req.headers.cookie || '';
        // Cookie NAME=length breakdown only (never values/tokens) so we can see what bloats the jar.
        let cookieMap = {};
        try { cookieRaw.split(';').forEach(c => { const i = c.indexOf('='); const n = (i < 0 ? c : c.slice(0, i)).trim(); if (n) cookieMap[n] = (c.length - n.length); }); } catch (_) {}
        // Also which large request-header NAMES are present (authorization / sentinel), lengths only.
        const bigHdrs = {};
        try { for (const [hk, hv] of Object.entries(req.headers)) { if (/authorization|sentinel|oai-|openai-/i.test(hk)) bigHdrs[hk.toLowerCase()] = String(hv).length; } } catch (_) {}
        safeLog('relay', { request_path: reqPathOnly, upstream_status: uRes.statusCode, req_hdr_bytes: reqHdrBytes, req_cookie_bytes: cookieRaw.length, cookies: cookieMap, big_hdrs: bigHdrs });
      }
      res.on('error', (e) => safeLog('res_error', { request_path: reqPathOnly, code: (e && e.code) || 'err' }));
      uRes.on('error', (e) => safeLog('ures_error', { request_path: reqPathOnly, code: (e && e.code) || 'err' }));
      res.on('close', () => { if (!res.writableEnded) safeLog('res_aborted', { request_path: reqPathOnly, upstream_status: uRes.statusCode }); });
      ...
      if (isHtml) {
        ...
        uRes.on('end', () => {
          let html = maybeDecompress(Buffer.concat(buf), uRes.headers['content-encoding']).toString('utf8');
          ...
          if (!ctx.asset && !isCaptchaReq && !cfPassthrough) {
            ...
            // Very LAST <head> insert → runs FIRST: install the cookie-jar minimizer before
            // any app/telemetry script can bloat document.cookie past LiteSpeed's header cap.
            html = injectCookieMinimizer(html, ctx);
          }
        ...
        uRes.on('data', c => buf.push(c));
        uRes.on('end', () => {
          let body = maybeDecompress(Buffer.concat(buf), uRes.headers['content-encoding']).toString('utf8');
        ...
        // Piped asset (JS/CSS/img/font/wasm/SSE): pass the body through byte-for-byte,
        // KEEPING the upstream compression so the browser inflates it (the transfer stays
        // small). content-encoding/length were stripped when outHeaders was built; re-add
        // them here so the compressed bytes decode correctly. (SSE has neither → no-op.)
        if (uRes.headers['content-encoding']) outHeaders['content-encoding'] = uRes.headers['content-encoding'];
        if (uRes.headers['content-length']) outHeaders['content-length'] = uRes.headers['content-length'];
        logIt(false);
        res.writeHead(uRes.statusCode || 200, outHeaders);
        uRes.pipe(res);
      }
...
// ── WebSocket proxy ──────────────────────────────────────────────────────────
// ChatGPT opens a WebSocket for realtime/stream delivery (e.g. /backend-api/celsius/
// ws/*). Node DESTROYS any upgrade socket when there is no 'upgrade' listener →
// ERR_CONNECTION_CLOSED in the browser, which stalls the chat send. Tunnel the upgrade
// to the upstream using the SAME lease/vault-session auth path as normal requests.
server.on('upgrade', async (req, socket, head) => {
  const pathOnly = String(req.url || '').split('?')[0];
  const bail = () => { try { socket.destroy(); } catch (_) {} };
  socket.on('error', () => {});
  try {
    const token = getLease(req);
    if (!token) return bail();
    const local = verifyLeaseLocal(token);
    if (local === null) return bail();
    const capture = !!(local && local.cap);
    let session;
    if (capture) session = { noAccount: true, capture: true };
    else {
      session = await getSession(token, local && local.jti);
      if (session && session.blocked) return bail();
    }
    const upURL = new URL(TARGET_ORIGIN);
    const upLib = upURL.protocol === 'https:' ? https : http;
    const headers = buildUpstreamHeaders(req, upURL, session, false);
    // Re-add the WebSocket handshake headers (stripped as hop-by-hop for normal reqs).
    ...
    headers.host = upURL.host;
    const upReq = upLib.request(`${upURL.origin}${req.url}`, { method: req.method || 'GET', headers });
    upReq.on('upgrade', (upRes, upSocket, upHead) => {
      const hdrLines = [];
      for (const [k, v] of Object.entries(upRes.headers)) {
        (Array.isArray(v) ? v : [v]).forEach(val => hdrLines.push(k + ': ' + val));
      }
      const head101 = 'HTTP/1.1 101 ' + (upRes.statusMessage || 'Switching Protocols') + '\r\n' + hdrLines.join('\r\n') + '\r\n\r\n';
      try { socket.write(head101); } catch (_) { return bail(); }
      if (upHead && upHead.length) { try { socket.write(upHead); } catch (_) {} }
      if (head && head.length) { try { upSocket.write(head); } catch (_) {} }
      upSocket.pipe(socket);
      socket.pipe(upSocket);
      const kill = () => { try { upSocket.destroy(); } catch (_) {} try { socket.destroy(); } catch (_) {} };
      upSocket.on('error', kill); socket.on('error', kill);
      upSocket.on('close', kill); socket.on('close', kill);
      safeLog('ws_open', { request_path: pathOnly, upstream_status: 101 });
    });
    upReq.on('response', (upRes) => { safeLog('ws_no_upgrade', { request_path: pathOnly, upstream_status: upRes.statusCode }); bail(); });
    upReq.on('error', (e) => { safeLog('ws_error', { request_path: pathOnly, code: (e && e.code) || 'err' }); bail(); });
    upReq.setTimeout(UPSTREAM_TIMEOUT_MS, () => { try { upReq.destroy(new Error('ws_timeout')); } catch (_) {} });
    upReq.end();
  } catch (e) {
    safeLog('ws_exception', { request_path: pathOnly, code: (e && e.code) || 'err' });
    bail();
  }
});
