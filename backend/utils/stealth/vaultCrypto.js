...
function vaultKey() {
  ...
  if (!secret) throw new Error('STEALTH_VAULT_KEY or JWT_SECRET is required to derive the vault key');
  return crypto.createHmac('sha256', secret).update('stealthwriter:vault:v1').digest();
}
