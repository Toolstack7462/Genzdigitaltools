'use strict';
/**
 * Robust StealthWriter cookie-bundle parsing + Cookie-header building.
 *
 * Accepts whatever an admin pastes and normalizes to a canonical bundle:
 *   { cookies: [{ name, value, domain?, path? }], localStorage?, sessionStorage?, origin? }
 *
 * Supported inputs:
 *   - JSON object { cookies: [...]|"raw", localStorage: {...}, ... }
 *   - JSON array of cookie objects  [{name,value,domain,...}, ...]  (browser export)
 *   - JSON array of pairs           [["name","value"], ...]
 *   - flat JSON map                 { "name": "value", ... }
...
// Domain match: cookie domain ".stealthwriter.ai" applies to host "stealthwriter.ai" / "app.stealthwriter.ai".
function hostMatchesCookieDomain(cookieDomain, host) {
  if (!cookieDomain) return true; // host-only cookie (no domain) — include
...
/** Build "name=value; name2=value2" for the upstream target host. Last value wins per name. */
