'use strict';
/**
 * BypassGPT — standalone reverse-proxy gateway (deploy on bypassgpt1.genzdigitalstore.com).
 *
 * Self-contained and SEPARATE from StealthWriter and from BypassGPT: its own target
 * origin, its own lease cookie, and the backend scopes its cookie vault by tool=bypassgpt.
 ...
 *      cookie, redirects to BypassGPT's default path.
 *   2. Validates the lease on EVERY request (signature + expiry locally; backend
 *      /validate on HTML page loads).
 *   3. Reverse-proxies to https://www.bypassgpt.ai, attaching the selected vault account's
...
const TOOL_KEY = process.env.TOOL_KEY || 'bypassgpt';
const TOOL_NAME = process.env.TOOL_NAME || 'BypassGPT';
const TARGET_ORIGIN = (process.env.BYPASSGPT_TARGET_ORIGIN || process.env.TARGET_ORIGIN || 'https://www.bypassgpt.ai').replace(/\/$/, '');
const DEFAULT_PATH = cleanPath(process.env.BYPASSGPT_DEFAULT_PATH || process.env.DEFAULT_PATH, '/ai-humanizer');
const SIGNIN_PATH = cleanPath(process.env.SIGNIN_PATH, '/sign-in');
const PUBLIC_ORIGIN = (process.env.GATEWAY_PUBLIC_ORIGIN || process.env.BYPASSGPT_GATEWAY_URL || 'https://bypassgpt1.genzdigitalstore.com').replace(/\/$/, '');
